...
func init() {
	...
	if length < 224 / 8 {
		logger.Error("The length of Rand Key is too small, Crypt init failed, Please reset it again !")
		return
	}
	if length >= 521/8 + 8 {
		logger.Info("Rand length = %v, Using 521 level!", length)
		curve = elliptic.P521()
	} else if length >= 384/8 + 8 {
		logger.Info("Rand length =", length, "Using 384 level !")
		curve = elliptic.P384()
	} else if length >= 256/8 + 8 {
		logger.Info("Rand length =", length, "Using 256 level !")
		curve = elliptic.P256()
	} else if length >= 224/8 + 8 {
		logger.Info("Rand length =", length, "Using 244 level !")
		curve = elliptic.P224()
	}
	...
	if err != nil {
		logger.Error("Crypt init fail,", err, " need = ", curve.Params().BitSize)
		return
	}
	puk = prk.PublicKey
}
